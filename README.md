...
Requires Go 1.24.4 or newer. The only dependency is `go.yaml.in/yaml/v3`.
...
| `Config.Extra` | Inline map for vendor-specific top-level keys not modeled by `Config`. |

`Render` always validates first, so a successful render is always a schema-shaped document.

### Vendor extensions

Top-level keys outside the modeled schema go in `Extra`, which is inlined into the rendered
document:

```go
cfg := cloudinit.Config{
	Hostname: "web-01",
	Extra: cloudinit.RawObject{
		"vendor_data_hook": map[string]any{"enabled": true},
	},
}
```

Extension names must be non-empty and must not collide with a module `Config` already supports —
validation reports `extra.<name>: conflicts with a supported module` if they do.
